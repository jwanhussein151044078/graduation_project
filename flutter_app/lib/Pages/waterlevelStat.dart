...
class waterlevelStat extends StatefulWidget {
  @override
  _waterlevelStatState createState() => _waterlevelStatState();
}

class _waterlevelStatState extends State<waterlevelStat> {
  List<data> l = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text("water level Statistics"),
          centerTitle: true,
  ...
  Future<void> get_current() async {
    var db = refdatabase.child("SWB/current");
    await db.once().then((DataSnapshot snapshot) {
      //print(snapshot.value);
      current =  (snapshot.value/100).floor()%100;
    });
    setState((){});
  }
}

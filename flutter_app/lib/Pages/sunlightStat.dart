...
class sunlightStat extends StatefulWidget {
  @override
  _sunlightStatState createState() => _sunlightStatState();
}

class _sunlightStatState extends State<sunlightStat> {
  List<data> l = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("SunLight Statistics"),
        centerTitle: true,
  ...
  Future<void> getData()async{
    ...
    await db.once().then((DataSnapshot snapshot) {
      for(int i = (counter+1)%SIZE, w= 0 ; w<SIZE ; w++,i=(i+1)%SIZE ){
        ...
        l.add(data(val: ((snapshot.value[i.toString()])/10000).floor(),time:(w)));
      }
    });
    setState(() {});
  }
  ...
  Future<void> get_current() async {
    ...
    await db.once().then((DataSnapshot snapshot) {
      //print(snapshot.value);
      current =  (snapshot.value/10000).floor();
    });
    setState(() {});
  }
}
